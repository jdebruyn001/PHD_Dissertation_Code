...
class TransformerModel(nn.Module):
    def __init__(self, input_vocab_size, target_vocab_size, device, embed_dim=256, num_heads=8, num_encoder_layers=6, num_decoder_layers=6, hid_dim=512, max_seq_length=100):
        super(TransformerModel, self).__init__()
        self.device = device
        print(f"embed_dim: {embed_dim}, num_heads: {num_heads}, num_encoder_layers: {num_encoder_layers}, num_decoder_layers: {num_decoder_layers}, hid_dim: {hid_dim}")
        self.transformer = nn.Transformer(d_model=embed_dim, nhead=num_heads,
                                  num_encoder_layers=num_encoder_layers,
                                  num_decoder_layers=num_decoder_layers,
                                  dim_feedforward=hid_dim)

        self.src_tok_emb = nn.Embedding(input_vocab_size, embed_dim)
        self.tgt_tok_emb = nn.Embedding(target_vocab_size, embed_dim)
        self.positional_encoding = PositionalEncoding(embed_dim, dropout=0.1, max_len=max_seq_length)
        self.generator = nn.Linear(embed_dim, target_vocab_size)
        self.init_weights()
...
# Train, Validate, Test Loop Adjustments
...
def test_loop(model, test_loader_loop, criterion, device, global_vocab):
    ...
    with torch.no_grad():
        for inputs, targets, names, _ in test_loader_loop:
            ...
            outputs = model(inputs, targets, src_mask=None)
...
def write_results(epoch, phase, metrics_str, round_name):
    directory = 'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/'
...
def main():
    ...
    model_save_directory = r'C:\Users\jaco\OneDrive\Desktop\PHD\Python\Downloads\TNN'
   ...
   # Defines a structured collection of data rounds for sequential processing. 
    ...
    for round_data in rounds:
        ...
        train_save_path = f'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/{round_name}_train_combined.xlsx'
        validate_save_path = f'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/{round_name}_validate_combined.xlsx'
        test_save_path = f'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/{round_name}_test_combined.xlsx'
        ...
        model = TransformerModel(input_vocab_size=input_dim, target_vocab_size=output_dim, device=device, 
                         embed_dim=256, num_heads=8, num_encoder_layers=6, num_decoder_layers=6, 
                         hid_dim=512, max_seq_length=100)

        optimizer = optim.Adam(model.parameters(), lr=0.0005)
        pad_token_idx = vocabulary['<pad>']
        loss_fn = nn.CrossEntropyLoss(ignore_index=pad_token_idx)
        padding_token = '<pad>'
        padding_index = vocabulary[padding_token] 
        src = torch.randint(high=vocab_size, size=(10, 32), dtype=torch.long, device=device)
        tgt = torch.randint(high=vocab_size, size=(20, 32), dtype=torch.long, device=device) 
        ...
        print(f"Max token ID in training dataset: {max_id_train}")
        print(f"Max token ID in validation dataset: {max_id_valid}")
        print(f"Max token ID in test dataset: {max_id_test}")
        ...
        for epoch in range(n_epochs):
            # Training
            model.train()
            train_loss, train_accuracy, train_precision, train_recall, train_f1, train_bleu = process_function(
                model=model,
            ...
            # Validation
            model.eval()
            valid_loss, valid_accuracy, valid_precision, valid_recall, valid_f1, valid_bleu = process_function(model, valid_loader, optimizer, loss_fn, device, CLIP, global_vocab, is_train=False)
        ...
        test_save_path = f'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/{round_name}_testing_results.txt'
        ...
        test_save_path = f'C:/Users/jaco/OneDrive/Desktop/PHD/Python/Downloads/TNN/{round_name}_global_test_results.txt'
